...
| DEV-04 | MLflow tracking (`tracking/mlflow_utils.py`, lineage tags) is implemented in M4 instead of M5. The untuned M4 runs are tagged `run_type=baseline`, not `candidate`. No tuning is pulled forward. | Requested in the M4 implementation brief. The distinct `run_type` keeps DOC-03 MAC-05 (3 `candidate` runs + 1 `final`) unaffected when M5 adds tuned candidate runs. | DOC-03 §6.1, MAC-05; DOC-05 M4, M5, §9 |
| DEV-05 | Default `mlflow.tracking_uri` is `sqlite:///mlflow.db` (artifacts in `./mlruns`) instead of `file:./mlruns`. | MLflow 3.16 refuses the file-store backend unless `MLFLOW_ALLOW_FILE_STORE=true`. DOC-03 §5/§6 already allow SQLite; `mlflow.db` is git-ignored per DOC-03 §3. Inspect with `mlflow ui --backend-store-uri sqlite:///mlflow.db`. | DOC-03 §5, §6; DOC-05 §7, §20 |
| DEV-06 | M4 persists baseline outputs to `reports/baseline/` (`training_summary.json`, `model_comparison.json`, `metrics/<model>.json`, `predictions/<model>_val.csv`) instead of keeping pipelines in memory only. Train/validation metrics only; `test.csv` and `temporal_holdout.csv` are not read (confirmed with the project owner). | Requested in the M4 brief for machine-readable, deterministic outputs. Kept separate from the M6 outputs (`reports/model_comparison.csv` etc.) so later milestones are unaffected. No fitted models are persisted (candidate `.joblib` files remain M5). | DOC-05 M4 Outputs |
| DEV-07 | M5 tuning runs from its own entrypoint `python -m fraud_detection.pipeline.tune` instead of inside `pipeline/train.py`. Besides the documented `models/candidates/{lr,rf,xgb}.joblib`, it writes `reports/tuning/{tuning_summary.json, tuning_results.csv, best_model.json}`. `best_model.json` names the leader by validation PR-AUC, marked provisional. | Requested in the M5 implementation brief; keeps the completed M4 baseline stage unchanged. The documented single `train` DVC stage (DOC-03 §4.2) still applies: M6/M8 must run tuning, selection and threshold optimisation within that stage. `best_model.json` does not replace the DOC-02 §12 selection in M6. | DOC-03 §4.2; DOC-05 M5, M6, §8 |
| DEV-08 | Each CV trial is also logged as a nested MLflow run (`run_type=tuning_trial`, CV scores only) under its `run_type=candidate` run. | Requested in the M5 brief ("log every tuning run"). Trials carry CV scores only; they are never scored on validation, so validation cannot influence hyperparameter choice. MAC-05 counts `candidate`/`final` runs and is unaffected. | DOC-03 §6.1, MAC-05 |
| DEV-09 | M6 threshold optimisation, selection and freezing run from `python -m fraud_detection.pipeline.select`, which loads the M5 candidates, instead of inside `pipeline/train.py`. Also: figures go to `reports/figures/` (`threshold_analysis_<model>.png`, `val_confusion_matrix_<model>.png`); PR-curve thresholds outside (0, 1) are excluded so a frozen threshold always satisfies DOC-04 §4 check 4; tuned-threshold metrics are appended to the M5 candidate runs, which are found by the SHA-256 of their logged `.joblib`. | Keeps M4 (`pipeline.train`) and M5 (`pipeline.tune`) unchanged and avoids re-tuning (about 15 minutes) on every selection run. The documented single `train` DVC stage (DOC-03 §4.2, MD-05) is kept by giving it the multi-command `cmd` `python -m fraud_detection.pipeline.tune` then `python -m fraud_detection.pipeline.select` (M8). Its outputs are exactly DOC-03 §4.2 row 3. **Approved by the project owner after M6 review.** | DOC-03 §4.2, §6.1; DOC-05 M6, §8 |
| DEV-10 | The `git_dirty` lineage tag ignores changes under `paths.reports_dir` and `paths.models_dir` (recorded in the tag `git_dirty_excludes`). Changes to code, config, docs, tests or any other tracked or untracked file still mark a run dirty. M6 now captures its lineage at the start of the step, before writing outputs, as M4 and M5 already did. | Generated outputs are rewritten by the pipeline itself (e.g. `train_metadata.json` holds the new MLflow run id and `model_version` holds the commit SHA, and multi-threaded Random Forest CV scoring can change last digits). Counting them would mark every run-of-record dirty even when the code and config exactly match the commit. From M8 these outputs are DVC-managed (DOC-03 §3), so the effective check is unchanged. | DOC-03 §3, §6; DOC-05 §9 |

**Decisions recorded after M6 review (project owner):**

| ID | Decision | Reason |
|---|---|---|
| DEC-01 | `r_min = 0.80` is **confirmed** (resolves DOC-02 TD-12) and fixed for the rest of the project; `threshold.r_min_confirmed: true` in config. The optional DOC-02 §13 stability rule ("prefer a flat region") is **not** adopted. Recorded before any test evaluation, as DOC-05 M6 requires. | On validation all three candidates reach recall 0.810 (51 of 63 frauds) at their tuned thresholds. The selected XGBoost reaches precision 0.981 (1 FP) at threshold 0.911154, a usable operating point. The threshold sits at the upper edge of a recall plateau (at 0.92 recall is 0.794). This is accepted as a known limitation: M7 test recall may fall below 0.80 by one or two frauds and must be reported as-is, without retuning. |
| DEC-02 | No M4/M5 reruns are required after the Random Forest scoring determinism fix; last-digit differences in Random Forest outputs are accepted. | Selection and threshold are unaffected (XGBoost selected). |
| DEC-03 | The large `reports/threshold_analysis_<model>.csv` tables stay generated artifacts: git-ignored now, DVC-managed from M8. | Several MB each; DOC-03 §3 routes large generated reports to DVC. |

**M7 implementation decisions and deviations (approved by the project owner after M7 review):**

| ID | Item | Reason | Affected documents |
|---|---|---|---|
| DEC-04 | Optional bootstrap CIs are **enabled**: `evaluation.bootstrap: {enabled: true, n: 1000, confidence: 0.95}`, seeded with `project.seed`, for PR-AUC, Recall and Precision on the test set and the temporal holdout. | DOC-02 §21.6 treats "differences within the bootstrap CI" as inconclusive; with 63 and 52 frauds, point estimates alone cannot separate shift from noise. DOC-05 §23 lists "bootstrap CI inclusion" as an implementation-time decision. | DOC-02 §21.5-21.6; DOC-05 §7 |
| DEC-05 | §21.6 "inconclusive" rule implemented as: a decrease is inconclusive if the two 95% bootstrap CIs overlap, or (for recall) if it amounts to at most `evaluation.inconclusive_fraud_cases` = 2 holdout fraud cases; otherwise it is reported as evidence of possible short-horizon temporal/distribution shift. | Makes the documented rule deterministic and testable. CI overlap is conservative; the report states this explicitly. | DOC-02 §21.6 |
| DEV-11 | `evaluate` also writes `reports/figures/test_pr_curve.png` and `test_roc_curve.png` (with the frozen operating point) besides the documented `test_confusion_matrix.png`. | Requested in the M7 brief; additive figures. | DOC-02 §14 step 5; DOC-03 §4.2 row 4 |
| DEV-12 | `evaluate_temporal` also reads `reports/temporal/temporal_split_summary.json` (M3) for development-pool prevalence and the low-confidence flag; M8 must add it as a dependency of that stage. | Needed for the holdout-vs-pool prevalence comparison (DOC-01 L-03, M7 brief). It holds only counts, prevalence and time ranges, no rows. | DOC-03 §4.2 row 5; DOC-05 §8 |
| DEV-13 | `model_config.json` holds the DOC-02 §15 and DOC-03 §7 keys plus `r_min`, `model_sha256` and `evaluation_lineage` (code/config lineage of the two evaluation steps). `trained_at` is the recorded start time of the MLflow run that trained and refitted the selected model (M5), not the evaluation time, so the file stays byte-identical across reruns. | Additive traceability; the no-timestamp reproducibility requirement is met while keeping the documented `trained_at` field. | DOC-02 §15; DOC-03 §7 |
| DEV-14 | DOC-02 §14 step 6 ("document final configuration in `reports/model_selection.md`") is met by `models/model_config.json` and `reports/temporal/temporal_robustness.md`; `model_selection.md` is not rewritten by the evaluation stages. | `model_selection.md` is a `train`-stage output; DVC forbids a second stage writing it (DOC-03 §4.2), and the evaluation stages must not modify train outputs. | DOC-02 §14; DOC-03 §4.2 |
| DEC-06 | M7 finding: on the temporal holdout the frozen model reaches recall 0.7500 (39 of 52 frauds), 3 fraud cases short of `r_min` = 0.80; test recall is 0.8254 (52 of 63, +1 case). All holdout decreases (PR-AUC -0.1165, recall -0.0754, precision -0.0299) are inconclusive under DEC-05 (overlapping 95% CIs). Accepted as a documented limitation and finding only: the model, threshold, `r_min`, training process and earlier milestone outputs are **not** changed in response (DOC-02 §21.6). | This confirms the plateau-edge risk recorded in DEC-01. DOC-01 SC-10 sets no minimum temporal performance. The M7 evaluation runs stay `git_dirty=true`; the evaluation of record is the clean `dvc repro` in M8. | DOC-01 SC-10; DOC-02 §21.6; DOC-05 M7-M8 |

**M8 implementation decisions and deviations (approved by the project owner after M8 review):**

| ID | Item | Reason | Affected documents |
|---|---|---|---|
| DEV-15 | `.gitattributes` forces LF line endings for text files in every checkout (`* text=auto eol=lf`; PNG/joblib/db binary). Working-tree files that Git had written with CRLF were rewritten from the index as LF, with no content changes. The M1 `validation_report.json` writer now forces LF too (the only Git-tracked output that did not); `data/interim/clean.csv` stays CRLF on Windows (DVC-cached, not a Git checkout; recorded as a risk). | DVC 3 hashes dependency files byte-for-byte. With a system-wide `core.autocrlf=true`, a Windows clone would check out CRLF, so every code/config dependency would differ from `dvc.lock` and MAC-03 ("`dvc status` clean in a fresh clone") would fail. | DOC-03 §8, MAC-02/03; DOC-05 §8 step 9 |
| DEV-16 | Small git-tracked reports that are neither metrics nor plots (`reports/model_comparison.csv`, `reports/model_selection.md`, `reports/tuning/tuning_results.csv`, `reports/temporal/temporal_vs_random_comparison.csv`, `reports/temporal/temporal_robustness.md`) are declared as `outs` with `cache: false`. Pipeline figures (`reports/figures/*.png`, `reports/temporal/temporal_confusion_matrix.png`) move from Git to the DVC cache (`git rm --cached`; files kept). EDA figures stay in Git because EDA is not a stage. | DOC-03 §3 keeps small human-readable reports in Git only as `cache: false` outputs and lists `*.png` as not Git-tracked; `cache: false` outs have the same effect as metrics/plots for `.md`/`.csv` tables. | DOC-03 §3; DOC-05 §8 step 4 |
| DEV-17 | `git_dirty` is content-based: `git diff HEAD` (line-ending normalised) plus untracked, non-ignored files, instead of `git status`; `dvc.lock` is always excluded (extends DEV-10; tag `git_dirty_excludes`). Real edits, staged or not, still mark a run dirty. | `dvc repro` rewrites `dvc.lock` after every stage and, with `--force`, rewrites `data/raw/creditcard.csv.dvc` with CRLF on Windows (same content). `git status` lists both, so every run of record would otherwise be tagged `git_dirty=true`. | DOC-03 §6; DEV-10 |
| DEC-07 | Reproduction of record, in two commits: (1) commit the M8 code, config, docs, tests, `dvc.yaml`, `.gitattributes`, `.dvcignore`, `requirements.txt` and the untracking of pipeline PNGs; (2) reset `mlflow.db`/`mlruns/`, run the M4 baseline (`python -m fraud_detection.pipeline.train`, not a stage: DEV-06), then `dvc repro --force --no-run-cache`; commit `dvc.lock`, the regenerated Git-tracked reports and the DVC-written `.gitignore` files; `dvc push`; tag `v1.0-model` on that commit. `model_version` then names commit (1), the code that produced the artifacts (MD-13). `reports/baseline/model_comparison.json` is declared as a dependency of `train` because tuning reads it for the baseline comparison. | Stages log to MLflow, so they must execute rather than be restored from the DVC run cache, and the store must start empty for MAC-05 (3 candidate + 1 final run; plus 3 baseline and 24 tuning-trial runs from DEV-04/DEV-08). | DOC-03 §6, §8, MAC-03, MAC-05; DOC-05 M8 |
| DEC-08 | `requirements.txt` fixed without changing any pin: re-encoded from UTF-16 to UTF-8; `pywin32==312` gets the marker `; sys_platform == "win32"`; the editable line `-e git+…@2bdb5d3#egg=fraud_detection` is removed (the package is installed with `pip install -e .`, DOC-05 M0 step 5). | Portable, pinned installs for Linux/Docker/CI (NFR-008, AC-20, MAC-09). `requirements-serve.txt` remains an M10 item. | DOC-05 M0, M10, AC-20; DOC-03 MAC-09 |

**M9 implementation decisions and deviations (approved by the project owner after M9 review):**

| ID | Item | Reason | Affected documents |
|---|---|---|---|
| DEV-18 | Where the M9 brief and DOC-04 differ, DOC-04 is implemented: `GET /health` returns `{"status": "ok", "model_loaded": true, "model_name", "model_version"}` (not `{"status": "healthy"}`); `POST /predict` returns all six DOC-04 §5.2 fields (`fraud_probability`, `prediction`, `label`, `threshold`, `model_name`, `model_version`); startup runs the full DOC-04 §4 checks 1-8 in order. | DOC-04 is authoritative for serving; each DOC-04 contract is a superset of what the brief asked for. | DOC-04 §4, §5 |
| DEV-19 | The single decision rule `apply_threshold` moves from `evaluation/metrics.py` to `fraud_detection/inference/predictor.py`; `metrics.py` re-exports it and `evaluate.py` imports it from inference. `evaluation/threshold.py` keeps its vectorised decision counting (thousands of candidate thresholds at once), with a test asserting it agrees with `apply_threshold` at every candidate threshold. | DOC-04 §6 requires the `>=` rule to be shared code in `fraud_detection.inference`, so evaluation and serving cannot diverge. Calling `apply_threshold` per threshold in M6 would be orders of magnitude slower for identical results. | DOC-04 §6; DOC-05 M9 |
| DEV-20 | `dvc.yaml`: `train`, `evaluate` and `evaluate_temporal` depend on `src/fraud_detection/inference` (via `evaluation/metrics.py`). | Code dependencies must cover every imported module (M8 test); otherwise a change to the decision rule would not invalidate these stages. Results are unchanged (identical rule), but those stages show as changed in `dvc status` until reproduced. | DOC-03 §4.2; DOC-05 §8 |
| DEV-21 | `pyproject.toml`: pytest `pythonpath` adds `"."` (so tests import the top-level `app` package) and registers the `artifact` marker (brought forward from M11). `tests/conftest.py` gains the shared serving fixtures: a tiny pipeline on synthetic data plus a matching `model_config.json`, and a sample transaction. | The FastAPI package lives at the repository root (DOC-03 §11). The fixture artifact pair is what DOC-04 §12 specifies, shared by `test_inference.py` and `test_api.py`. | DOC-04 §12; DOC-05 M11 |
| DEV-22 | Test dependencies `httpx==0.28.1` and `httpcore==1.0.9` added to `requirements.txt`. They are not serving dependencies and stay out of `requirements-serve.txt` (M10). | FastAPI's `TestClient` (DOC-04 §12) requires `httpx`; pip added exactly these two packages. | DOC-02 §16; DOC-05 M0, M10, AC-20 |
| DEC-09 | A failed prediction returns `500 {"detail": "Prediction failed"}`; the server log records the exception type and traceback but never the request payload, feature values or predictions. Other unhandled errors return `500 {"detail": "Internal server error"}`. Validation errors return 422 in FastAPI's standard `{"detail": [...]}` shape with `loc`, `msg` and `type` only: submitted values are never echoed (FastAPI's default echoes them, which also turned NaN/Infinity inputs into a 500 because NaN cannot be serialised). | DOC-04 §7. Library exception messages normally carry no feature values; this is a residual, accepted risk. | DOC-04 §7, §11 |
| DEC-10 | The fresh-clone reproduction check (DOC-03 MAC-03; DOC-05 AC-19; M8 task "reproduction check from a fresh clone") is deferred to M12, where DOC-05 §8 step 9 schedules it (fresh clone → `dvc pull` → `dvc repro` → `dvc status` clean, `dvc metrics diff` within tolerance). | The in-place M8 verification already showed `dvc repro --force` reproduces every output byte-identically except lineage fields; a single fresh-clone reproduction at M12 covers M8 and the final state. | DOC-03 MAC-03; DOC-05 M8, §8 step 9, AC-19 |
| DEC-11 | M9 is committed before the M8 run of record, so the single reproduction of record (DEC-07), `dvc.lock`, MLflow lineage and `model_version` come from the final serving codebase. DR-02/DR-03/DR-04 against the real model, and the `artifact`-marked tests, are verified after that run restores `models/model.joblib`, `models/model_config.json` and `dvc.lock`. | M9 changes files that `train`, `evaluate` and `evaluate_temporal` depend on (DEV-19/20); reproducing before M9 would leave `dvc.lock` stale. The M9 code is fully covered by fixture-artifact tests (76 passed). | DOC-04 §13 DR-02..04; DOC-05 M8, M9 |

**M10 implementation decisions and deviations (approved by the project owner after M10 review):**

| ID | Item | Reason | Affected documents |
|---|---|---|---|
| DEV-23 | `requirements-serve.txt` pins the complete Linux runtime closure of the eight DOC-04 §8 packages (24 packages, every pin identical to `requirements.txt`), and the Dockerfile installs it with `pip install --no-cache-dir --no-deps`. xgboost's Linux-only multi-GPU dependency `nvidia-nccl-cu13` is deliberately omitted. A build step imports `app.main`, sklearn, xgboost, pandas, numpy, joblib and uvicorn so an incomplete closure fails the build. `imbalanced-learn` is not included (the frozen model is not the SMOTE pipeline). | Without `--no-deps`, pip would add unpinned transitive packages, including the several-hundred-MB NCCL library, which CPU inference never uses. Fully pinning keeps the image reproducible and small (DOC-03 MAC-09). | DOC-04 §8; DOC-05 M10, §14 |
| DEV-24 | `.dockerignore` is an allow-list: `*`, then re-include only `requirements-serve.txt`, `app/`, `src/fraud_detection/__init__.py`, `src/fraud_detection/inference/`, `models/model.joblib`, `models/model_config.json`; caches are always excluded. It excludes everything in the DOC-05 §14 list (with `src/fraud_detection/eda/` per DEV-02, and `venv/` as well as `.venv/`) plus anything added later. | An allow-list cannot silently let new files (e.g. a new data or report folder) into the image. | DOC-04 §8; DOC-05 §14, DEV-02 |
| DEV-25 | Base image pinned to `python:3.14.7-slim` (training Python recorded in `model_config.json`); non-root user `appuser` (uid 10001); stdlib `HEALTHCHECK` on `/health`; `MODEL_PATH`/`MODEL_CONFIG_PATH` set to `/app/models/...`. `tests/test_docker.py` adds static checks: serve pins identical to `requirements.txt`, no training/tooling packages, complete `--no-deps` closure, base image matches the training Python, only allowed `COPY` sources, runtime settings, allow-list `.dockerignore`. | DOC-04 §8 items marked [REC]/[EMP] resolved; the tests keep the three M10 files consistent with DOC-04/05 as dependencies evolve. | DOC-04 §8, DD-11; DOC-03 MAC-09 |
| DEC-12 | The image was built and verified with the M9 fixture artifact pair (the real `models/model.joblib` is restored by the pending M8 run of record, DEC-11). Docker runs inside WSL 2 (Ubuntu) on this machine. DR-05/DR-06/DR-08 with the real model (image tag = `model_version`, `/predict` parity with local uvicorn, image contents) are verified after the run of record. | DOC-04 §8 build precondition (`models/` populated); the image mechanics (dependencies, startup checks, endpoints, validation, contents, non-root, healthcheck) do not depend on which frozen model is copied in. | DOC-04 §13 DR-05/06/08; DOC-05 M10 |
| DEV-26 | Image tag = `model_version` with `+` replaced by `_` (e.g. `1.0.0+abc1234` → `fraud-detector:1.0.0_abc1234`); the exact `model_version` is passed as build arg `MODEL_VERSION` and stored in the OCI label `org.opencontainers.image.version`. DR-08 is checked against the label and `/health`. | Docker tag syntax does not allow `+` (DOC-03 MD-13 uses SemVer build metadata), so the literal DOC-04 §9 command `docker build -t fraud-detector:<model_version>` fails. | DOC-04 §9, DR-08; DOC-03 MD-13 |
